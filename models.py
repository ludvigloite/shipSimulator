...
class SeachartSimulationConfiguration(NamedTuple):
    size_of_map_east: int
    size_of_map_north: int
    center_of_map_east: int
    center_of_map_north: int
    database_file_names: list[str]
    new_data: bool
    border: bool
    route_txt: str
    waypoint_color: str
    draft_of_ship: int
    safety_margin: int
    wind_arrow_drawing_coefficient: int
    current_arrow_drawing_coefficient: int
    current_wind_arrow_offset: int
    wind_arrow_color: str
    current_arrow_color: str
    able_to_crash:bool
    conservation_areas_txt: str
...
class ShipMachineryModel(BaseMachineryModel):
    ...
    def integrate_differentials(self):
        ...
        '''
        self.omega = self.sat(self.int.integrate(x=self.omega, dx=self.d_omega),0,self.shaft_speed_max)
        #TODO: Check if the saturation is valid here. It was not there in Børges version.
...
class BaseShipModel:
    def __init__(
            ...
            seachart_config: SeachartSimulationConfiguration
    ):
        ...
        self.current_velocity_component_from_north = environment_config.current_velocity_component_from_north
        self.current_velocity_component_from_east = environment_config.current_velocity_component_from_east
        ...
        # Initialize seacharts
        size = seachart_config.size_of_map_east, seachart_config.size_of_map_north
        self.center = seachart_config.center_of_map_east, seachart_config.center_of_map_north
        #size = 18000, 10124
        #center = 36580, 6960000

        self.enc = seacharts.ENC(size=size, center=self.center, files=seachart_config.database_file_names, new_data=seachart_config.new_data, border=seachart_config.border)
        self.draft_of_ship = seachart_config.draft_of_ship
        self.safety_margin = seachart_config.safety_margin
        
        with open(seachart_config.route_txt) as file:
            for line in file:
                north, east = line.rsplit()
                self.enc.draw_circle((int(east), int(north)), 200, seachart_config.waypoint_color, thickness=2, fill=False)

        with open(seachart_config.conservation_areas_txt) as file:
            for line in file:
                east, north, width, height = line.rsplit()
                self.enc.draw_rectangle((int(east), int(north)), (int(width), int(height)), 'red', rotation=0)


        self.shoreGeometry = self.enc.shore.geometry
        self.landGeometry = self.enc.land.geometry

        self.depths = []
        for key, _ in self.enc.seabed.items():
            self.depths.append(key)
        
        self.seabedList = {}
        seabedGeo = self.enc.seabed
        for i in range(len(self.depths)-1):
            self.seabedList[self.depths[i]] = seabedGeo[self.depths[i]].geometry-seabedGeo[self.depths[i+1]].geometry
        self.seabedList[self.depths[-1]]= seabedGeo[self.depths[-1]].geometry

        self.ship_id = 1
        self.ship_positions = []

        self.wind_arrow_drawing_coefficient = seachart_config.wind_arrow_drawing_coefficient
        self.current_arrow_drawing_coefficient = seachart_config.current_arrow_drawing_coefficient
        self.current_wind_arrow_offset = seachart_config.current_wind_arrow_offset
        self.wind_arrow_color = seachart_config.wind_arrow_color
        self.current_arrow_color = seachart_config.current_arrow_color
        
        self.able_to_crash = seachart_config.able_to_crash
    ...
    def three_dof_kinetics(self, *args, **kwargs):
        ...
        wave_force = np.array([0, 0, 0]) #Probably shouldnt be 0?
...
class ShipModel(BaseShipModel):
    ...
    def __init__(self, ship_config: ShipConfiguration, simulation_config: SimulationConfiguration,
                 environment_config: EnvironmentConfiguration, seachart_config: SeachartSimulationConfiguration,
                 machinery_config: MachinerySystemConfiguration, initial_propeller_shaft_speed_rad_per_s):
        super().__init__(ship_config, simulation_config, environment_config, seachart_config)
        ...
        self.max_rudder_angle = machinery_config.max_rudder_angle_degrees
        self.simulation_results = defaultdict(list)
        self.consequence_dicts = []
        self.failure_mode_running = False
        self.seachart_config = seachart_config
    ...
    def store_simulation_data(self, load_perc, rudder_angle):
        ...
        self.simulation_results['Throttle [?]'].append(load_perc)
        ...
        if self.failure_mode_running == False:
        ...
        self.simulation_results['max rudder angle [deg]'].append(self.max_rudder_angle)
        self.simulation_results['min rudder angle [deg]'].append(-self.max_rudder_angle)
        self.simulation_results['max propeller shaft speed [rpm]'].append(self.ship_machinery_model.shaft_speed_max * 30 / np.pi)

    def simulate_failure(self, failure_type, nu_of_timesteps, failure_time_length, auto_pilot, throttle_controller, desired_forward_speed_meters_per_second, ship_color, rudder_angle, throttle, failures, printAllFailures, probabilityNet, damageStateNet, ship_info, cost_function_weights, draw_ships = True, draw_safety_margin_violations = True):
        
        def getDamageState():
            speed = (self.forward_speed**2 + self.sideways_speed**2)**0.5
            if speed*1.94 < 5:
                damageStateNet.set_evidence("Speed","Under_5_knots")
            elif speed*1.94 < 15:
                damageStateNet.set_evidence("Speed","Between_5_15_knots")
            else:
                damageStateNet.set_evidence("Speed","Over_15_knots")

            damageStateNet.update_beliefs()
            return damageStateNet.get_node_value("Damage_State")

        def calculateEnvConsequence(damageStateProbabilities, sensitiveArea = False):
            envConsequence = 0
            for i in range(len(damageStateProbabilities)):
                c_env = 300 if sensitiveArea else 120
                q_area = 100 * (i+1)
                C_impact = c_env*q_area
                

                if ship_info["ship_type"] == "Oil_tanker" and i>1:
                    Q_spill = ship_info["tank_size"]*(i-1)
                    C_spill = 24020*Q_spill**0.8447
                else:
                    C_spill = 0


                envConsequence += (C_impact+C_spill)*damageStateProbabilities[i]

            return envConsequence


        def calculateSocialConsequence(damageStateProbabilities):
            socialConsequence = 0
            for i in range(len(damageStateProbabilities)):
                N_ll = 0.01 if i>1 else 0
                V_ll = 3*10**6
                C_LL = N_ll*V_ll

                C_Inj =2*60000
                socialConsequence += (C_LL+C_Inj)*damageStateProbabilities[i]
            return socialConsequence

        def calculateEconomicConsequence(damageStateProbabilities):
            economicConsequence = 0
            damage_state_dict = {1: (0.01, 7), 2: (0.02, 14), 3: (0.025, 21), 4: (0.04, 30), 5: (0.1, 40), 6: (0.2, 50)}

            for i in range(len(damageStateProbabilities)):
                relative_damage, repair_time = damage_state_dict.get(i+1, (None, None))

                C_RC = relative_damage*ship_info["value"]

                C_LE = repair_time*ship_info["daily_profit"]

                if ship_info["ship_type"]=="Oil_tanker" and i>1:
                    Q_spill = ship_info["tank_size"]*(i-1)
                    C_LC = Q_spill*ship_info["oil_price"]
                else:
                    C_LC = 0

                economicConsequence += (C_RC+C_LE+C_LC)*damageStateProbabilities[i]
            return economicConsequence



        
        self.failure_mode_running = True
        
        saved_time_step = self.int.time
        saved_yaw_angle = self.yaw_angle
        saved_forward_speed = self.forward_speed
        saved_sideways_speed = self.sideways_speed
        saved_yaw_rate = self.yaw_rate
        saved_omega = self.ship_machinery_model.omega
        saved_d_north = self.d_north
        saved_d_east = self.d_east
        saved_d_yaw = self.d_yaw
        saved_d_omega = self.ship_machinery_model.d_omega
        saved_north = self.north
        saved_east = self.east
        saved_max_shaft_speed = throttle_controller.max_shaft_speed
        saved_rudder_angle = rudder_angle
        saved_throttle = throttle
        saved_ship_error_i = throttle_controller.ship_speed_controller.error_i
        saved_shaft_error_i = throttle_controller.shaft_speed_controller.error_i
        saved_pid_error_i = auto_pilot.heading_controller.ship_heading_controller.error_i
        saved_pid_prev_error = auto_pilot.heading_controller.ship_heading_controller.prev_error
        saved_navigation_e_ct_int = auto_pilot.navigate.e_ct_int
        saved_next_wpt = auto_pilot.next_wpt
        saved_prev_wpt = auto_pilot.prev_wpt
        saved_able_to_crash = self.able_to_crash

        if failure_type == 'FM1: Full Blackout':
            throttle_controller.max_shaft_speed = 0
            self.ship_machinery_model.shaft_speed_max = 0
            
            printAllFailures and print("Failure: Full Power blackout for",nu_of_timesteps,"seconds at time",int(self.int.time))
        elif failure_type == 'FM2: 80% Power Loss':
            throttle_controller.max_shaft_speed = 0.2*saved_max_shaft_speed
            self.ship_machinery_model.shaft_speed_max = 0.2*saved_max_shaft_speed
            
            printAllFailures and print("Failure: 80% Power Loss for",failure_time_length,"seconds at time",int(self.int.time),". Simulating",nu_of_timesteps,"seconds")
        elif failure_type == 'FM3: 50% Power Loss':
            throttle_controller.max_shaft_speed = 0.5*saved_max_shaft_speed
            self.ship_machinery_model.shaft_speed_max = 0.5*saved_max_shaft_speed
            
            printAllFailures and print("Failure: 50% Power Loss for",failure_time_length,"seconds at time",int(self.int.time),". Simulating",nu_of_timesteps,"seconds")
        elif failure_type == 'FM4: Rudder Freeze':

            printAllFailures and print("Failure: Rudder Freeze for",failure_time_length,"seconds at time",int(self.int.time),". Simulating",nu_of_timesteps,"seconds")
        ...
        consequence_dict = {}
        consequence_dict['failure_mode'] = failure_type
        consequence_dict['starting_time'] = saved_time_step
        consequence_dict['worst_violation'] = ''
        consequence_dict['total_consequence'] = 0
        consequence_dict['risk'] = 0
        consequence_dict['probability'] = 0
        consequence_dict['consequences'] = {'envConsequence' : 0, 'socialConsequence' : 0, 'economicConsequence' : 0}
        consequence_dict['damage_state'] = [0 for i in range(6)]
        self.able_to_crash = True

        safetyMarginDrawed = False


        while self.int.time < nu_of_timesteps+saved_time_step:
            ...
            if failure_type!= 'FM1: Full Blackout' and self.int.time >= saved_time_step+failure_time_length:
                throttle_controller.max_shaft_speed = saved_max_shaft_speed
                self.ship_machinery_model.shaft_speed_max = saved_max_shaft_speed

            # Find appropriate rudder angle and engine throttle
            if failure_type != 'FM4: Rudder Freeze' or self.int.time > saved_time_step+failure_time_length:
            ...
            dist_land = position.distance(self.landGeometry)
            dist_shore = position.distance(self.shoreGeometry)

            minimum_distance_to_hazard = min(dist_land, dist_shore)

            for i in range(len(self.depths)):
                if self.depths[i] <= self.draft_of_ship:
                    dist_seabed = position.distance(self.seabedList[self.depths[i]])
                    if dist_seabed < minimum_distance_to_hazard:
                        minimum_distance_to_hazard = dist_seabed

            if minimum_distance_to_hazard <= self.safety_margin:
                if draw_safety_margin_violations and not safetyMarginDrawed:
                    self.enc.draw_circle((east_position,north_position), 100, 'orange', thickness=3, fill=False)
                    safetyMarginDrawed = True
                consequence_dict['worst_violation'] = 'did_violate_safety_margin'
            if minimum_distance_to_hazard == 0 and self.able_to_crash:
                sensitive_area = False

                with open(self.seachart_config.conservation_areas_txt) as file:
                    for line in file:
                        east, north, width, height = line.rsplit()
                        east, north, width, height = int(east), int(north), int(width), int(height)
                    if (east-width <= east_position <= east+width) and (north-height <= north_position <= north+height):
                            sensitive_area = True

                damageStateProbabilities = getDamageState()

                envConsequence = calculateEnvConsequence(damageStateProbabilities, sensitiveArea=sensitive_area)
                socialConsequence = calculateSocialConsequence(damageStateProbabilities)
                economicConsequence = calculateEconomicConsequence(damageStateProbabilities)

                totalConsequence = cost_function_weights['alpha_env']*envConsequence+cost_function_weights['alpha_soc']*socialConsequence+cost_function_weights['alpha_eco']*economicConsequence

                probabilityNet.update_beliefs()
                blackoutUtily = probabilityNet.get_node_value("Blackout_utility_metric")[0]                
            
                if printAllFailures:
                    print("CRASH!")
                consequence_dict['worst_violation'] = 'did_crash'
                consequence_dict['total_consequence'] = totalConsequence
                consequence_dict['probability'] = blackoutUtily * ship_info['blackout_base_probability']
                consequence_dict['risk'] = totalConsequence * consequence_dict['probability']
                consequence_dict['consequences'] = {'envConsequence' : envConsequence, 'socialConsequence' : socialConsequence, 'economicConsequence' : economicConsequence}
                consequence_dict['damage_state'] = damageStateProbabilities

                self.add_vessel_drawing(east_position=east_position, north_position=north_position, heading=heading, ship_color=ship_color)
                self.enc.draw_circle((east_position,north_position), 50, 'red', thickness=10, fill=False)
                break
            
            # Update and integrate differential equations for current time step
            self.store_simulation_data(throttle, rudder_angle)
            ...
            times.append(self.int.time)

            # Make a drawing of the ship from above every 20 second
            if draw_ships and time_since_last_ship_drawing > 30:
                self.add_vessel_drawing(east_position=east_position, north_position=north_position, heading=heading, ship_color=ship_color)
                self.ship_snap_shot()
                time_since_last_ship_drawing = 0
            time_since_last_ship_drawing += self.int.dt
            # Progress time variable to the next time step
            self.int.next_time()

        #If ship crash, self.int.time<nu_of_timesteps
        failures.append([saved_time_step, self.int.time-saved_time_step, failure_type])
        consequence_dict['ending_time'] = self.int.time

        self.int.time = saved_time_step
        self.yaw_angle = saved_yaw_angle
        self.forward_speed = saved_forward_speed
        self.sideways_speed = saved_sideways_speed
        self.yaw_rate = saved_yaw_rate
        self.ship_machinery_model.omega = saved_omega
        self.d_north = saved_d_north
        self.d_east = saved_d_east
        self.d_yaw = saved_d_yaw
        self.ship_machinery_model.d_omega = saved_d_omega
        self.north = saved_north
        self.east = saved_east
        throttle_controller.max_shaft_speed = saved_max_shaft_speed
        self.ship_machinery_model.shaft_speed_max = saved_max_shaft_speed
        rudder_angle =  saved_rudder_angle
        throttle = saved_throttle
        throttle_controller.ship_speed_controller.error_i = saved_ship_error_i
        throttle_controller.shaft_speed_controller.error_i = saved_shaft_error_i
        auto_pilot.heading_controller.ship_heading_controller.error_i = saved_pid_error_i
        auto_pilot.heading_controller.ship_heading_controller.prev_error = saved_pid_prev_error
        auto_pilot.navigate.e_ct_int = saved_navigation_e_ct_int
        auto_pilot.next_wpt = saved_next_wpt
        auto_pilot.prev_wpt = saved_prev_wpt
        self.able_to_crash = saved_able_to_crash
        self.failure_mode_running = False

        self.consequence_dicts.append(consequence_dict)
        

        return throttle_controller, auto_pilot, failures
    ...
    def add_vessel_drawing(self, east_position, north_position, heading, ship_color):
        self.ship_id += 1
        self.ship_positions.append((self.ship_id, int(east_position), int(north_position), math.degrees(heading), ship_color))        

    def draw_wind_and_current_arrows(self):
        if self.wind_speed:
            self.enc.draw_arrow((self.center), (self.center[0] - math.sin(self.wind_dir)*self.wind_speed*self.wind_arrow_drawing_coefficient, self.center[1] - math.cos(self.wind_dir)*self.wind_speed*self.wind_arrow_drawing_coefficient), self.wind_arrow_color,
                        head_size=150, width=25, thickness=2)

        if self.current_velocity_component_from_east or self.current_velocity_component_from_north:
            self.enc.draw_arrow((self.center[0],self.center[1]+self.current_wind_arrow_offset), (self.center[0] + self.current_velocity_component_from_east*self.current_arrow_drawing_coefficient, self.center[1] + self.current_velocity_component_from_north*self.current_arrow_drawing_coefficient+self.current_wind_arrow_offset), self.current_arrow_color,
                        head_size=150, width=25, thickness=2)
